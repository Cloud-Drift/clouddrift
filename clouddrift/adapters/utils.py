...
def download_with_progress(download_map: List[_DownloadRequest], prewrite_func = _ID_FUNC):
...
@retry(
    retry=retry_if_exception(lambda ex: isinstance(ex, [requests.Timeout, requests.HTTPError])),
    wait=wait_exponential_jitter(initial=0.25),
    stop=stop_after_attempt(10)
)
def _download_with_progress(url: str, output: Union[BufferedIOBase, str],
    ...
    if isinstance(output, str) and os.path.exists(output):
        ...
        with requests.head(url, timeout=5) as response:
            if "Last-Modified" in response.headers:
                ...
                warnings.warn("Cannot determine the file has been updated on the remote source. \
    ...
    try:
        ...
        if buffer is not None \
                and not isinstance(buffer, BufferedIOBase) \
                or force_close:

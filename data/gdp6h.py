...
def rowsize(index: int) -> int:        
    return xr.open_dataset(join(folder, dict_id[index], file_pattern.format(id=index)), decode_cf=False, decode_times=False, concat_characters=False, decode_coords=False).dims['obs']
...
def preprocess(index: int) -> xr.Dataset:
    ...
    ds = xr.load_dataset(join(folder, dict_id[index], file_pattern.format(id=index)), decode_times=False, decode_coords=False)
    ...
    for var in ['longitude', 'latitude', 'err_lat', 'err_lon', 've', 'vn', 'temp', 'err_temp']:
    ...
    vars_attrs = {
        ...
        'temp': {'long_name': 'Fitted sea water temperature', 'units':'Kelvin', 'comments': 'Estimated near-surface sea water temperature from drifting buoy measurements. It is the sum of the fitted near-surface non-diurnal sea water temperature and fitted diurnal sea water temperature anomaly. Discrepancies may occur because of rounding.'},
        'err_temp': {'long_name': 'Standard uncertainty of fitted sea water temperature', 'units':'Kelvin', 'comments': 'Estimated one standard error of near-surface sea water temperature estimate from drifting buoy measurements'},

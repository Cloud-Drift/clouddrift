...
# parse the csv file
df = pd.read_csv(join(folder, file), delimiter='\s+', header=5, 
...
def preprocess(index: int) -> xr.Dataset:   
    ...
    return xr.Dataset(
        data_vars=dict(
            ...
            err_pos=(['obs'], df_subset.err_pos, {'long_name': 'estimated position error', 'units':'m'}),
            ve=(['obs'], df_subset.ve, {'long_name': 'Eastward velocity', 'units':'m/s'}),
            vn=(['obs'], df_subset.vn, {'long_name': 'Northward velocity', 'units':'m/s'}),
            err_vel=(['obs'], df_subset.err_vel, {'long_name': 'Standard error in latitude', 'units':'degrees_north'}),    
        ),

        coords=dict(
            longitude=(['obs'], df_subset.lon, {'long_name': 'Longitude', 'units':'degrees_east'}),
            latitude=(['obs'], df_subset.lat, {'long_name': 'Latitude', 'units':'degrees_north'}),
            time=(['obs'], df_subset.datetime, {'long_name': 'Time'}),
            ids=(['obs'], df_subset.index, {'long_name': "Buoy ID for all observations", 'units':'-'}),
        ),

...
def test_analytic_transform_imag_odd(self):
    ...
    self.assertTrue(
        np.allclose(z - np.mean(z, keepdims=True), zp + np.conj(zn), atol=0.1, rtol=0)
    )
...
def test_analytic_transform_ndarray(self):
    ...
    self.assertTrue(
        np.allclose(
            x - np.mean(x, axis=0, keepdims=True), np.real(z0), atol=0.1, rtol=0
        )
    )
    self.assertTrue(
        np.allclose(
            x - np.mean(x, axis=1, keepdims=True), np.real(z1), atol=0.1, rtol=0
        )
    )
    self.assertTrue(
        np.allclose(
            x - np.mean(x, axis=2, keepdims=True), np.real(z2), atol=0.1, rtol=0
        )
    )
    self.assertTrue(
        np.allclose(
            x - np.mean(x, axis=-1, keepdims=True), np.real(z3), atol=0.1, rtol=0
        )
    )

from clouddrift.sphere import (
    ...
    earthsphere_to_cartesian,
    cartesian_to_earthsphere,
    EARTH_RADIUS_METERS,
)
import unittest
import numpy as np
...
class earthsphere_to_cartesian_tests(unittest.TestCase):
    def test_earthsphere_to_cartesian(self):
        lon = np.array([0, 90, 0, -90, 0]).astype(np.double)
        lat = np.array([0, 0, 45, 45, -90]).astype(np.double)
        x, y, z = earthsphere_to_cartesian(lon, lat, earth_radius=1)
    ...
    def test_earthsphere_to_cartesian_invert(self):
        lon = np.random.uniform(size=100) * 360
        lat = np.random.uniform(size=100) * 180 - 90
        x, y, z = earthsphere_to_cartesian(lon, lat)
        lon2, lat2 = cartesian_to_earthsphere(x, y, z)
        self.assertTrue(np.allclose(lon, np.mod(lon2, 360)))
        self.assertTrue(np.allclose(lat, lat2))


class cartesian_to_earthsphere_tests(unittest.TestCase):
    def test_cartesian_to_earthsphere(self):
        ...
        lon_expected = np.array([0, 90, 180, -90, 0, 0])
        lat_expected = np.array([0, 0, 0, 0, 90, -90])
        lon, lat = cartesian_to_earthsphere(x, y, z)
        self.assertTrue(np.allclose(lon, lon_expected))
        self.assertTrue(np.allclose(lat, lat_expected))

    def test_cartesian_to_earthsphere_invert(self):
        x = np.random.random(size=100)
        y = np.random.random(size=100)
        z = np.random.random(size=100)
        lon, lat = cartesian_to_earthsphere(x, y, z)
        x2, y2, z2 = earthsphere_to_cartesian(lon, lat, earth_radius=1)

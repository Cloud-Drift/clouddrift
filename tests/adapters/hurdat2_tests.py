    ...
    def test_basin_selection(self) -> None:
        with self.subTest("Select Atlantic Basin"):
            download_requests = hurdat2._get_download_requests(
                hurdat2.BasinOption.ATLANTIC, "some_path"
            )
        ...
        with self.subTest("Select Pacific Basin"):
            download_requests = hurdat2._get_download_requests(
                hurdat2.BasinOption.PACIFIC, "some_path"
            )
            url, _, _ = download_requests[0]
        ...
        with self.subTest("Select Both Option"):
            download_requests = hurdat2._get_download_requests(hurdat2.BasinOption.BOTH, "some_path")
